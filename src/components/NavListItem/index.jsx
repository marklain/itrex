...
const NavListItem = ({ name, id, allGrades, setActiveGrade, onDeleteGrade }) => {
    ...
    <li className="grade-navigation__grade">
        <NavLink onClick={() => {setActiveGrade(id, allGrades)}} activeClassName="grade-navigation__link-active" className="grade-navigation__link" to={`/grades/${name}`}>GRADE {name}</NavLink>
